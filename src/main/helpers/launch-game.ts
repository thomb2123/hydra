...
const prepareLinuxCompatibilityForLaunch = async (
  ...
  const requestedWinePrefixPath = hasOnlineFixFiles(parsedPath)
    ? path.join(process.env.HOME ?? "", "SteamPrefixes", "480")
...
const launchResolvedGame = async (
  ...
  if (isWindowsExecutable(parsedPath)) {
    // Existing Steam shortcuts own their prefix; never use them for a cloud restore.
    if (
      !(await canRunAutomaticCloudSaveSync(objectId, shop)) &&
      (await launchOnlineFixThroughSteam(null, parsedPath))
    ) {
      PowerSaveBlockerManager.markCompatibilityLaunchStarted(gameKey);
      return null;
    }
